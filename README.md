# Japan Immigration Statistics (Monthly)

日本の地方出入国在留管理局ごとの月次「受理・処理・滞留（pending）」と、年度別の永住許可人数を、
e-Stat（政府統計の総合窓口）の公開統計から整理し、構造化 JSON として配布するデータセットです。

> 公開統計のみを扱います。出典：e-Stat（https://www.e-stat.go.jp）。

整形済みデータ：`public-data.json`

CDN（jsDelivr）経由でも取得できます：
```
...
6. 提交后 jsDelivr CDN 约 12h 内刷新到最新。
...
- **双口径**：`monthly` = 纯辖区（东京减成田/羽田/横浜，用于推算+默认展示）；
  `bureauTotal` = 管内合计（e-Stat 原始列，仅对比展示）。
- **calibrationFactor**：按 积压÷处理量 硬算的等待时间偏短，乘此系数对齐实际观测。
